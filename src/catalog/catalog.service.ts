...
export class CatalogService {
  ...
  async getAchievementDiaries(): Promise<AchievementDiaryResponseDto[]> {
    const achievementDiaries = await this.achievementDiaryRepo.find({
      select: { id: true, area: true, tier: true, name: true, slug: true },
      order: { area: 'ASC', tier: 'ASC', name: 'ASC' },
    });

    return achievementDiaries.map((achievementDiary) => ({
      id: achievementDiary.id,
      area: achievementDiary.area,
      tier: achievementDiary.tier,
      name: achievementDiary.name,
      slug: achievementDiary.slug,
    }));
  }
}
